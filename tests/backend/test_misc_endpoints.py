...
class TestDemandEndpoints:
    ...
    def test_demand_forecast_skus_exist_in_inventory(self, client):
        """Test that every demand forecast maps to a real inventory item."""
        forecasts = client.get("/api/demand").json()
        inventory = client.get("/api/inventory").json()
        inventory_by_sku = {item["sku"]: item for item in inventory}

        # Forecasts must be priceable - unit_cost only exists on inventory records
        for forecast in forecasts:
            assert forecast["item_sku"] in inventory_by_sku, \
                f"Forecast SKU {forecast['item_sku']} does not exist in inventory"
            assert forecast["item_name"] == inventory_by_sku[forecast["item_sku"]]["name"], \
                f"Forecast name for {forecast['item_sku']} does not match the inventory record"

    def test_demand_forecast_trend_mix(self, client):
        """Test that demand forecasts cover every trend direction."""
        response = client.get("/api/demand")
        data = response.json()

        trends = {item["trend"].lower() for item in data}

        for expected in ["increasing", "stable", "decreasing"]:
            assert expected in trends, f"No demand forecast with '{expected}' trend"
